...
class Ball:
    ...
    def draw(self):
        ...
        if pos[1]<=0: #천장에 닿음 -> 아래로 방향 전환
            ...
            self.y=abs(self.y) #'방향'에만 초점 맞추기 위해 abs 사용
        ...
        if not self.hit_bottom:
            if self.hit_paddle(pos) == True: #패들과 부딪혔는지 검사
                ...
                # 일정한 점수 간격(5로 설정)마다 속도 증가 (난이도 조절)
                speed_up=1 #기본 설정
                if self.game.score%5==0:
                    speed_up*=1.2 #5점마다 20% 증가

                #최대 속도 설정(무한대로 속도 증가 방지)
                max_speed=10

                # 좌, 우 방향 설정
                self.x=self.x/abs(self.x)*abs(self.x)*speed_up 
                # 최대 속도 초과한 경우 속도 제한
                if abs(self.x)>max_speed:
                    self.x=self.x/abs(self.x)*max_speed

                # 위, 아래 방향 -> 패들에 튕길 때 위로 올라가도록 설정 ("-"방향)
                self.y=-abs(self.y)*speed_up
                if abs(self.y)>max_speed:
                    self.y=-max_speed
